...
def login():
	os.system("clear")
	cetak(nel(f'   {P2}Hi!  Looks like you are a new user.  Thank you for choosing this tool as your trusted choice.  Before Using These Tools, You Must Login First!  Don't Forget To Give The Best Rating On Github Yes!  Thank You!\n\n                {H2}- MAMUN -',title=f'{P2} {H2}[ {P2}Welcome User {H2}]',width=54,padding=(1,4),style='green'))
	cetak(nel(f'{P2} Input Cookie !\in Cookie Capture Extension Suggestion : {H2}Cookie Login',subtitle=f'{P2}┌─[ Input Cookie ]]',subtitle_align='left',width=54,padding=1,style='green'))
	cookie = input(f"{P}   └──> : {H}")
	try:
...
{P2} Current Date    : {hari}, {tanggal}''',title=f'{P2} {H2}[ {P2}User Information {H2}]',subtitle_align='left',padding=1,style='green'))
	...
	cetak(nel(f'{P2} INPUT PUBLIC POST LINK',subtitle=f'{P2}┌─',subtitle_align='left',width=25,padding=0,style='green'))
	link = input(f"{P}   └──> : {H}")
	cetak(nel(f'{P2} INPUT THE NUMBER OF SHARE',subtitle=f'{P2}┌─',subtitle_align='left',width=22,padding=0,style='green'))
	...
	try:
		...
		for x in range(jumlah):
			...
			if "id" in post:
				...
				print("\n")
				cetak(nel(f'{P2} AUTO SHARE STOP THE POSSIBILITY OF COOKIE INVALID OR SACRIFICE OFF',width=35,padding=0,style='red'));exit()
